...
HashEncrypt::HashEncrypt(const unsigned char *key, const unsigned char *iv, size_t ivSizeBytes)
{
    ...
    EVP_EncryptInit_ex(_pctx, EVP_aes_128_gcm(), NULL, NULL, NULL); //TODO check return value == 1

    EVP_CIPHER_CTX_ctrl(_pctx, EVP_CTRL_GCM_SET_IVLEN, ivSizeBytes, NULL);

    EVP_EncryptInit_ex(_pctx, NULL, NULL, _key, _iv);
...
void HashEncrypt::hashUpdate(unsigned char *in, int inSizeBytes)
{
    ...
    EVP_EncryptUpdate(_pctx, NULL, &_unusedOutl, in, inSizeBytes); //TODO check return value == 1
}
...
void HashEncrypt::hashFinal(unsigned char *out, unsigned int *outSizeBytes)
{
    ...
    EVP_EncryptFinal_ex(_pctx, NULL, &_unusedOutl); //TODO check return value == 1

    EVP_CIPHER_CTX_ctrl(_pctx, EVP_CTRL_GCM_GET_TAG, _finalSizeBytes, out);
